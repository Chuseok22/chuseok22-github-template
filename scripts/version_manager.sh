...
# - set: 특정 버전으로 설정
# - validate: 버전 형식 검증
# - parse_project_version: 프로젝트 타입별 버전 파싱 (CI/CD용)
#
# ===================================================================
...
parse_project_version() {
    ...
    # 결과 출력 (json 형식)
    echo "{\"version\":\"$version\",\"build_number\":\"$build_number\",\"current_version\":\"$current_version\"}"
}
...
main() {
    ...
    case "$command" in
        ...
        "parse_project_version")
            ...
            if [ -z "$version_info" ] || [ -z "$project_type" ]; then
                echo_error "사용법: $0 parse_project_version <version_info> <project_type>"
                exit 1
            fi
            
            parse_project_version "$version_info" "$project_type"
            ;;
        *)
            echo "사용법: $0 {get|increment|set|validate|parse_project_version} [options]"
            echo ""
            echo "Commands:"
            echo "  get                 - 현재 버전 가져오기"
            echo "  increment           - patch 버전 증가"
            echo "  set <version>       - 특정 버전으로 설정"
            echo "  validate [version]  - 버전 형식 검증"
            echo "  parse_project_version <version_info> <project_type> - 프로젝트 타입별 버전 파싱 (CI/CD용)"
            echo ""
            echo "Examples:"
            echo "  $0 get"
            echo "  $0 increment"
            ...
            echo "  $0 parse_project_version 1.2.3 spring"
            exit 1
            ;;
    esac
}
